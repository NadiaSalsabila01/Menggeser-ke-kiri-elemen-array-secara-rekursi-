...
ExpNode* left;
ExpNode* right;
ExpNode(string v) : value(v), left(NULL), right(NULL) {}
};


ExpNode* buatNode(string v) {
return new ExpNode(v);
}


ExpNode* buildTree(int harga, int jumlah) {
ExpNode* root = buatNode("*");
root->left = buatNode(to_string(harga));
root->right = buatNode(to_string(jumlah));
return root;
}


void inorder(ExpNode* r) {
if (!r) return;
if (r->left) cout << "(";
inorder(r->left);
cout << r->value;
...
int evaluasi(ExpNode* r) {
if (!r->left && !r->right) return stoi(r->value);
int l = evaluasi(r->left);
int rr = evaluasi(r->right);
return l * rr;
}


void hapusTree(ExpNode* r) {
if (!r) return;
hapusTree(r->left);
hapusTree(r->right);
delete r;
...
// ================= BINARY EXPRESSION TREE =================
ExpNode* treeTotal = buildTree(pilihan->harga, jumlahTiket);

cout << "\n=== BINARY EXPRESSION TREE TOTAL HARGA ===\n";
cout << "Inorder   : ";
inorder(treeTotal);
cout << endl;
...
// Evaluasi tree (harga * jumlah)
int totalTree = evaluasi(treeTotal);
cout << "Hasil Evaluasi Tree = Rp" << totalTree << endl;

// Gunakan hasil tree sebagai total resmi
total = totalTree;
